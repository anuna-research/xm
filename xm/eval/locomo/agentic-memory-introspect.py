#!/usr/bin/env python3
"""
LoCoMo Agentic Memory Evaluation with xm Introspection

Extended version that exposes xm's introspection capabilities:
- schema classes: List all node types with counts
- schema predicates: List all predicates with usage
- query backlinks: Find nodes linking TO a target (Org-roam style)
- query path: Find paths between nodes

This tests whether graph introspection improves retrieval accuracy.

Usage:
    export OPENAI_API_KEY=your_key
    python3 eval/locomo/agentic-memory-introspect.py --limit 20
"""
...
QUERY_TOOLS = [
    ...
    },
    {
        ...
        "description": "List all predicates (properties/relationships) used in the knowledge graph with usage counts. Use this to discover what properties are available for querying.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
    },
    {
        ...
        "description": "Get detailed description of a class or predicate, including example values.",
        "input_schema": {
            "type": "object",
            "properties": {
                "term": {
                    "type": "string",
                    "description": "The class or predicate URI to describe (e.g., 'xm:fact', 'xm:content')"
                }
            },
            "required": ["term"]
        }
    },
    {
        ...
        "description": "Find all nodes that link TO a specific node (Org-roam style backlinks). Use this to discover related information by following reverse connections.",
        "input_schema": {
            "type": "object",
            "properties": {
                "target": {
    ...
    {
        "name": "query_nodes",
        "description": "Search for nodes by type or properties.",
        "input_schema": {
            "type": "object",
            "properties": {
                "node_type": {
                    "type": "string",
                    "description": "Filter by node type (e.g., 'fact', 'agent', 'entity')"
                },
                "limit": {
                    "type": "integer",
...
class XmStore:
    ...
    # ========================================================================
    # Introspection Methods (using xm CLI)
    # ========================================================================

    def schema_classes(self) -> str:
        """List all classes (node types) with instance counts."""
    ...
    def schema_predicates(self) -> str:
        """List all predicates (properties) with usage counts."""
        success, output = self._run_xm(["schema", "predicates"])
        if success:
            return output
        return "Schema predicates not available"

    def schema_describe(self, term: str) -> str:
        """Describe a class or predicate."""
        success, output = self._run_xm(["schema", "describe", term])
        if success:
            return output
        return f"Could not describe: {term}"
    ...
    def query_nodes(self, node_type: str = None, limit: int = 20) -> str:
        ...
        if node_type:
            # SPARQL query for specific type
            sparql = f"""
            SELECT ?node ?content ?subject WHERE {{
                ?node a <https://xm.dev/ns/v1#{node_type}> .
                OPTIONAL {{ ?node <https://xm.dev/ns/v1#content> ?content }}
                OPTIONAL {{ ?node <https://xm.dev/ns/v1#subject> ?subject }}
            }} LIMIT {limit}
            """
            success, output = self._run_xm(["query", "sparql", sparql])
            if success:
        ...
        if node_type:
            results = [m for m in results if m["type"] == node_type]
...
def execute_query_tool(tool_name: str, tool_input: Dict) -> str:
    ...
    elif tool_name == "schema_describe":
        return MEMORY_STORE.schema_describe(tool_input.get("term", ""))
...
QUERY_SYSTEM = """You are a question-answering agent with access to a memory system stored in xm (a knowledge graph).

You have both SEARCH tools and INTROSPECTION tools:

**INTROSPECTION TOOLS** (use to explore the graph structure):
- schema_classes: See what types of nodes exist (fact, person, event, etc.)
- schema_predicates: See what properties are available (content, subject, timestamp)
- query_backlinks: Find nodes that link TO a target (discover related info)
- query_nodes: List nodes by type

**SEARCH TOOLS** (use to find specific memories):
